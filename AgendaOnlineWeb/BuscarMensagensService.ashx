...
public class BuscarMensagensService : IHttpHandler {
    ...
    {    
        ...
        var mensagens = from msgs in db.mensagem where (msgs.id_conversa.ToString() == conversaID) orderby msgs.dt_envio 
               select new MensagemVO{ Id = msgs.id.ToString(), Texto = msgs.texto, DataEnvio = msgs.dt_envio.Value, EhMensagemDoRemetente = msgs.id_usuario.ToString() != usuarioLogadoID};
